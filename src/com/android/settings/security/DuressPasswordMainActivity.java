...
public class DuressPasswordMainActivity extends DuressPasswordActivity
    ...
    private static final String KEY_HAS_ASKED_FOR_USER_CREDENTIALS =
            "asked_for_user_credentials";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        var recyclerLayout = new GlifRecyclerLayout(this);
        layout = recyclerLayout;
        recyclerLayout.setIcon(getDrawable(R.drawable.ic_lock));
        recyclerLayout.setHeaderText(R.string.duress_pwd_pref_title);
        adjustDescriptionStyle(recyclerLayout);
        recyclerLayout.setDescriptionText(R.string.duress_pwd_description);
        setContentView(recyclerLayout);

        if (savedInstanceState != null) {
            userCredential = savedInstanceState.getParcelable(
                    KEY_USER_CREDENTIAL, LockscreenCredential.class);
            askedForUserCredentials =
                    savedInstanceState.getBoolean(KEY_HAS_ASKED_FOR_USER_CREDENTIALS);
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(
                KEY_HAS_ASKED_FOR_USER_CREDENTIALS, askedForUserCredentials);
    }
    ...
    protected void onResume() {
        ...
        if (userCredential == null) {
            if (!getLockPatternUtils().isSecure(getUserId())) {
                ...
                var builder = new ChooseLockSettingsHelper.Builder(this);
                builder.setRequestCode(REQ_CODE_OBTAIN_USER_CREDENTIALS);
                builder.setReturnCredentials(true);
                builder.setForegroundOnly(true);
                builder.show();
    ...
    private void updateActionList() {
        ...
        if (getLockPatternUtils().hasDuressCredentials(userCredential)) {
            group.addChild(createItem(R.string.duress_pwd_action_update, R.drawable.ic_edit));
            group.addChild(createItem(R.string.duress_pwd_action_delete, R.drawable.ic_delete));
        } else {
            group.addChild(createItem(R.string.duress_pwd_action_add, R.drawable.ic_add_24dp));
        }

        var adapter = new RecyclerItemAdapter(group);
    ...
    private Item createItem(@StringRes int title, @DrawableRes int icon) {
        var item = new Item();
        item.setId(title);
        item.setTitle(getText(title));
        item.setIcon(getDrawable(icon));
        return item;
    }

    @Override
    public void onItemSelected(IItem selectedItem) {
        Item item = (Item) selectedItem;
        int id = item.getId();
        if (id == R.string.duress_pwd_action_add
                || id == R.string.duress_pwd_action_update) {
            var intent = new Intent(this, DuressPasswordSetupActivity.class);
            intent.putExtra(DuressPasswordSetupActivity.EXTRA_TITLE_TEXT, id);
            intent.putExtra(
                    DuressPasswordSetupActivity.EXTRA_USER_CREDENTIAL, userCredential);
            allowNextOnStop = true;
            startActivityForResult(intent, REQ_CODE_SETUP);
        } else if (id == R.string.duress_pwd_action_delete) {
            var builder = new AlertDialog.Builder(this);
            builder.setMessage(R.string.duress_pwd_action_delete_confirmation);
            builder.setPositiveButton(R.string.duress_pwd_delete_button,
                    (dialog, which) -> deleteDuressCredentials());
            builder.setNegativeButton(R.string.duress_pwd_cancel_button, null);
            builder.show();
        }
    }

    private void deleteDuressCredentials() {
        ...
        try {
            ...
            dialog.setMessage(getString(R.string.duress_pwd_delete_error, e.toString()));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        var credential = data.getParcelableExtra(
